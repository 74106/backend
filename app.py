...
def case_law():
    ...
    try:
        ...
        if not api_key or requests is None:
            return jsonify({
                'error': 'Case law provider not configured',
                'hint': 'Set INDIAN_KANOON_API_KEY on the server to enable court case search.'
            }), 501

        # Indian Kanoon search API
        # Docs: returns JSON with list under 'results'
        # Endpoint shape: https://api.indiankanoon.org/search/?formInput=<q>&pagenum=0
        try:
            ...
            if resp.status_code != 200:
                return jsonify({'error': f'Provider error HTTP {resp.status_code}'}), 502
            data = resp.json()
        except Exception as prov_err:
            logger.error(f"Kanoon search failed: {prov_err}")
            return jsonify({'error': 'Failed to query case law provider'}), 502
